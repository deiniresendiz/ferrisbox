...
type Mode = 'textToHex' | 'hexToText';

export const HexConverter: React.FC = () => {
  const { t } = useTranslation();
  const { isFavorite, addFavorite, removeFavorite } = useFavorites();
  const [mode, setMode] = useState<Mode>('textToHex');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');
  const [copied, setCopied] = useState(false);

  const toolId = 'hex-converter';
  const favorite = isFavorite(toolId);

  const handleProcess = async () => {
    try {
      if (mode === 'textToHex') {
        const result = await invoke<string>('string_to_hex_command', { text: input });
        setOutput(result);
      } else {
        const result = await invoke<string>('hex_to_string_command', { hex: input });
        setOutput(result);
      }
      setError('');
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">
            {t('tools.hexConverter.name')}
          </h2>
          <p className="text-gray-600 dark:text-gray-400">{t('tools.hexConverter.description')}</p>
        </div>
        <button
      ...
      <div className="mb-4 flex gap-2 flex-wrap">
        <div className="flex bg-gray-100 dark:bg-space-700 rounded-lg p-1">
          ...
          >
            {t('tools.hexConverter.tabs.textToHex')}
          </button>
          <button
          ...
          >
            {t('tools.hexConverter.tabs.hexToText')}
          </button>
        </div>
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          {t('tools.hexConverter.examples', 'Examples')}:
        </p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <textarea
            ...
            placeholder={
              mode === 'textToHex'
                ? t('tools.hexConverter.placeholder.text')
                : t('tools.hexConverter.placeholder.hex')
            }
