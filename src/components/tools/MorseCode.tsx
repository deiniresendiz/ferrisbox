...
export const MorseCode: React.FC = () => {
  ...
  const toolId = 'morse-code';
  const favorite = isFavorite(toolId);

  const handleProcess = async () => {
    try {
      if (mode === 'encode') {
        const result = await invoke<string>('encode_morse_command', { text: input });
        setOutput(result);
      } else {
        const result = await invoke<string>('decode_morse_command', { morse: input });
        setOutput(result);
      }
      setError('');
    } catch (err) {
  ...
  const examples =
    mode === 'encode'
      ? [
          ...
          { label: 'With numbers', value: 'CODE 123' },
          { label: 'With punctuation', value: 'HELLO, WORLD!' },
        ]
      : [
          { label: 'SOS', value: '... --- ...' },
          { label: 'HELLO WORLD', value: '.... . .-.. .-.. --- / .-- --- .-. .-.. -..' },
          { label: 'With numbers', value: '-.-. --- -.. . / .---- ..--- ...--' },
          {
            label: 'Punctuation',
            value: '.... . .-.. .-.. --- --..-- / .-- --- .-. .-.. -.. -.-.--',
          },
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">
            {t('tools.morseCode.name')}
          </h2>
          <p className="text-gray-600 dark:text-gray-400">{t('tools.morseCode.description')}</p>
        </div>
        <button
      ...
      <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-300">
          ...
          {mode === 'encode'
            ? t(
                'tools.morseCode.info.encode',
                'Words separated by " / ", letters by space. Supports A-Z, 0-9, and punctuation (.,?!\'";:()=+-_@&/)'
              )
            : t(
                'tools.morseCode.info.decode',
                'Use " / " to separate words and space to separate letters'
              )}
        </p>
      </div>
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          {t('tools.morseCode.examples', 'Examples')}:
        </p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <textarea
            ...
            placeholder={
              mode === 'encode'
                ? t('tools.morseCode.placeholder.text', 'Enter text to encode...')
                : t('tools.morseCode.placeholder.morse', 'Enter morse code (e.g., ... --- ...)')
            }
