...
export const UrlEncoder: React.FC = () => {
  ...
  const handleEncode = async () => {
    try {
      const result = await invoke<string>('encode_url_command', { text: input });
      setOutput(result);
      setError('');
    } catch (err) {
      setError(String(err));
  ...
  const handleDecode = async () => {
    try {
      const result = await invoke<string>('decode_url_command', { encoded: input });
      setOutput(result);
      setError('');
    } catch (err) {
      setError(String(err));
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mt-4 p-3 bg-gray-50 dark:bg-space-700 border border-gray-200 dark:border-space-600 rounded-lg">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          <strong>Note:</strong> This encoder uses percent-encoding (RFC 3986). Special characters
          like spaces become %20, etc.
        </p>
      </div>
    </div>
  );
