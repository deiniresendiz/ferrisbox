...
export const XmlFormatter: React.FC = () => {
  ...
  const handleFormat = async () => {
    try {
      const result = await invoke<string>('format_xml_command', {
        input,
        indent: indentSize,
      });
      setOutput(result);
  ...
  const handleMinify = async () => {
    try {
      const result = await invoke<string>('minify_xml_command', { input });
      setOutput(result);
      setError('');
      setIsValid(true);
    } catch (err) {
  ...
  const handleValidate = async () => {
    try {
      const result = await invoke<boolean>('validate_xml_command', { input });
      setIsValid(result);
      if (result) {
        setError('');
      } else {
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">
            {t('tools.xmlFormatter.name')}
          </h2>
          <p className="text-gray-600 dark:text-gray-400">{t('tools.xmlFormatter.description')}</p>
        </div>
        <button
      ...
      <div className="mb-4 flex gap-2 flex-wrap items-center">
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-700 dark:text-gray-300">
            {t('tools.xmlFormatter.indentSize', 'Indent')}:
          </label>
          <select
            value={indentSize}
        ...
        <button onClick={handleFormat} className="btn btn-primary">
          {t('tools.xmlFormatter.format', 'Format')}
        </button>
        <button onClick={handleMinify} className="btn btn-secondary">
          {t('tools.xmlFormatter.minify', 'Minify')}
        </button>
        <button onClick={handleValidate} className="btn btn-secondary">
          {t('tools.xmlFormatter.validate', 'Validate')}
        </button>
        <button onClick={clearAll} className="btn btn-secondary">
          {t('common.clear')}
        </button>
        <button onClick={() => setInput(exampleXml)} className="btn btn-secondary">
          {t('tools.xmlFormatter.example', 'Example')}
        </button>
        {output && (
          <button
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {t('tools.xmlFormatter.input', 'Input XML')}
          </label>
          <textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder={t('tools.xmlFormatter.placeholder', 'Paste your XML here...')}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {t('tools.xmlFormatter.output', 'Output')}
          </label>
          <textarea
            value={output}
            readOnly
