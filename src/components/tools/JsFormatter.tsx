...
export const JsFormatter: React.FC = () => {
  ...
  const toolId = 'js-formatter';
  const favorite = isFavorite(toolId);

  const handleFormat = async () => {
    if (!input.trim()) {
      setError(t('tools.jsFormatter.emptyInput'));
      setOutput('');
      setIsValid(null);
      return;
    }
    ...
    try {
      const result = await invoke<string>('format_js_command', {
        input,
        indent: indentSize,
      });
      setOutput(result);
  ...
  const handleMinify = async () => {
    ...
    try {
      const result = await invoke<string>('minify_js_command', { input });
      setOutput(result);
      setError('');
      setIsValid(true);
    } catch (err) {
  ...
  const handleValidate = async () => {
    ...
    try {
      await invoke<string>('validate_js_command', { input });
      setIsValid(true);
      setError('');
    } catch (err) {
      setError(String(err));
  ...
  const handleExample = () => {
    const example = `const fibonacci = (n) => {
if (n <= 1) return n;
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
              {t('tools.jsFormatter.name')}
            </h2>
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              {t('tools.jsFormatter.description')}
            </p>
          </div>
          <button
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('tools.jsFormatter.indentSize')}:
              </label>
              <select
                value={indentSize}
            ...
            >
              {isValid ? (
                <>
                  ...
                  <span className="font-medium">{t('tools.jsFormatter.validJs')}</span>
                </>
              ) : (
                <>
                  <XCircle className="w-5 h-5" />
                  <span className="font-medium">{t('tools.jsFormatter.invalidJs')}</span>
                </>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea
                ...
                placeholder={t('tools.jsFormatter.placeholder')}
            ...
            <div>
                  ...
                  >
                    {copied ? (
                      <>
                        ...
                        {t('tools.jsFormatter.copied')}
                      </>
                    ) : (
                      <>
                        <Copy className="w-4 h-4" />
                        {t('tools.jsFormatter.copy')}
                      </>
                    )}
                  </button>
                )}
              ...
              <textarea
                value={output}
                readOnly
                placeholder={t('tools.jsFormatter.outputPlaceholder')}
